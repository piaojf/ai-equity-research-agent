  ...
  return (
    <main className="shell">
      ...
      <section className="hero">
        <div><span className="eyebrow">Evidence-first equity intelligence</span><h1>Research that shows its work.</h1><p className="lead">A production-style workspace for market signals, deterministic scoring, SEC evidence and explainable AI research.</p><div className="stack" style={{ marginTop: 28 }}><Link className="button" href="/stock/NVDA">Explore NVDA</Link><div className="nav">{tickers.map((ticker) => <Link key={ticker} href={`/stock/${ticker}`} className="pill">{ticker}</Link>)}</div></div></div>
